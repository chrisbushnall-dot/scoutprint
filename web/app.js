...
function bindPlayerReportLinks(root=document){
  root.querySelectorAll('[data-dossier-id]:not([data-dossier-bound])').forEach(element=>{
    element.dataset.dossierBound="true";
    element.addEventListener("click",event=>{event.stopPropagation();openPlayerDossier(element.dataset.dossierId);});
    if(element.tagName!=="BUTTON")element.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.stopPropagation();openPlayerDossier(element.dataset.dossierId);}});
  });
  root.querySelectorAll('[data-player-name]:not([data-dossier-bound])').forEach(element=>{
    element.dataset.dossierBound="true";
    element.addEventListener("click",event=>{event.stopPropagation();openPlayerByName(element.dataset.playerName,element.dataset.playerClub);});
  });
}

async function openPlayerByName(name,club=""){
  if(!name||name==="Unknown")return;
  try{
    const query={player:name,unique_players:"true",minimum_minutes:"0",limit:"10"};if(club)query.club=club;
    let payload=await proxy("players",{query});if(!(payload.players||[]).length&&club){delete query.club;payload=await proxy("players",{query});}
    const normalized=String(name).trim().toLocaleLowerCase(),rows=payload.players||[],match=rows.find(row=>String(row.player_name||"").trim().toLocaleLowerCase()===normalized)||rows[0];
    if(!match){toast(`No Scoutprint report is available for ${name}.`);return;}
    await openPlayerDossier(match.player_season_id);
...
async function unlock(){
  ...
  try{await Promise.all([loadCatalogue(),findReferences("Mohamed Salah",true),loadRadarCatalogue()]);const requested=location.hash.slice(1),dossier=requested.startsWith("dossier=")?decodeURIComponent(requested.slice(8)):null;showProductView(dossier?"players":["radar","league","players","recruitment","teams","matches"].includes(requested)?requested:"radar");if(requested==="league")loadLeague();if(dossier){history.replaceState({dossier},"",`#dossier=${encodeURIComponent(dossier)}`);await openPlayerDossier(dossier,{pushHistory:false,returnView:"players"});}}catch(error){handleError(error);}
}

const radarModeCopy={
  "breakouts":["RADAR INTEREST","Breakouts","Collective current-player evidence across the latest three normalized seasons, weighted 50/30/20 by recency."],
  "biggest-risers":["DEVELOPMENT","Biggest Risers","Positive supported like-for-like movement, with a current transition required and three-season evidence weighted by recency."],
  "u21":["AGE CONTEXT","U21","Current players aged 21 or younger, ranked by transparent three-season Radar evidence."],
  "underlying-output":["PRODUCTION GAP","Underlying > Output","Current supported xG and xA evidence running ahead of recorded goals and assists."],
  "role-changes":["ROLE EVOLUTION","Role Changes","Current supported behaviour-led role changes between comparable seasons."]
};
...
async function loadRadarCatalogue(){
  ...
  $("#radar-season").innerHTML=`<option value="">Current 3-season view</option>${payload.seasons.map(item=>`<option value="${escapeHtml(item)}">${escapeHtml(item)}</option>`).join("")}`;
...
function renderLeague(payload){
  ...
  $("#league-leaderboards").innerHTML=leagueModes.map(id=>boards.find(board=>board.id===id)).filter(Boolean).map(board=>`<article class="league-board surface"><header><div><p class="kicker">${escapeHtml(board.sort.field.replaceAll("_"," "))}</p><h2>${escapeHtml(board.label)}</h2></div><span>${integer.format(board.available)} available</span></header>${board.players.length?`<ol>${board.players.map(row=>`<li><button class="player-report-link" type="button" data-dossier-id="${escapeHtml(row.player_season_id)}"><span>${escapeHtml(row.player_name)}</span><small>${escapeHtml(row.club||"Club unavailable")} · ${escapeHtml(row.primary_role||"Unclassified")}</small></button><strong>${leagueBoardValue(row,board.sort.field)}</strong></li>`).join("")}</ol>`:'<p class="league-board-empty">No supported players at this threshold.</p>'}</article>`).join("");bindPlayerReportLinks($("#league-leaderboards"));
}
...
function renderMatch(payload){const match=payload.match;$("#match-detail-context").textContent=`${match.date||"Date unavailable"} · ${match.league?.name||"League unavailable"} · ${match.round||"Round unavailable"}`;$("#match-detail-title").textContent=`${match.home_team?.name||"Home"} vs ${match.away_team?.name||"Away"}`;$("#match-detail-ground").textContent=[match.stadium,match.referee?`Referee: ${match.referee}`:null].filter(Boolean).join(" · ");$("#match-detail-score").textContent=`${match.score_home??"—"}–${match.score_away??"—"}`;$("#match-evidence-grid").hidden=false;$("#match-shot-summary").innerHTML=(payload.shot_summary||[]).map(item=>`<div><strong>${escapeHtml(item.team?.name||"Team")}</strong><span>${item.shots} shots · ${item.on_target} on target · ${value(item.xg,2)} xG · ${item.big_chances} big chances</span></div>`).join("");$("#match-shot-sequence").innerHTML=(payload.shots||[]).map(shot=>`<div><b>${shot.minute??"—"}'</b><button class="player-report-link" type="button" data-player-name="${escapeHtml(shot.player?.name||"Unknown")}" data-player-club="${escapeHtml(shot.team||"")}"><span>${escapeHtml(shot.player?.name||"Unknown")} · ${escapeHtml(shot.team||"")}</span><small>${escapeHtml(shot.outcome||"Shot")} · ${value(shot.xg,2)} xG</small></button></div>`).join("")||'<p class="safe-empty">Shot evidence is unavailable.</p>';$("#match-performers").innerHTML=(payload.top_performers||[]).map(row=>`<div><button class="player-report-link" type="button" data-player-name="${escapeHtml(row.name||"Unknown")}" data-player-club="${escapeHtml(row.team||"")}"><span>${escapeHtml(row.name||"Unknown")}</span><small>${escapeHtml(row.team||"")} · ${row.stats.minutes??"—"} min</small></button><strong>${value(row.stats.rating,2)}</strong></div>`).join("")||'<p class="safe-empty">Player ratings are unavailable.</p>';renderMatchLineups(payload);renderMatchPlayers(payload.player_stats||[]);renderMatchNetworks(payload.pass_networks||[]);$("#match-limitations").innerHTML=(payload.limitations||[]).map(item=>`<li>${escapeHtml(item)}</li>`).join("");bindPlayerReportLinks($("#match-evidence-grid"));requestAnimationFrame(()=>drawMatchShots($("#match-shot-map"),payload.shots||[],match));}
function renderMatchLineups(payload){const match=payload.match,lineups=payload.lineups||{};$("#match-lineup-grid").innerHTML=[[match.home_team,lineups.home],[match.away_team,lineups.away]].map(([team,side])=>`<section><header><strong>${escapeHtml(team?.name||"Team")}</strong><span>${escapeHtml(side?.formation||"Formation unavailable")}</span></header>${side?`<ol>${side.starters.map(player=>`<li><b>${escapeHtml(player.shirt_number||"—")}</b><button class="player-report-link" type="button" data-player-name="${escapeHtml(player.name||"Unknown")}" data-player-club="${escapeHtml(team?.name||"")}">${escapeHtml(player.name||"Unknown")}${player.captain?" (c)":""}</button></li>`).join("")}</ol><small>Coach: ${escapeHtml(side.coach||"Unavailable")} · ${side.substitutes.length} substitutes</small>`:'<p class="safe-empty">Lineup unavailable.</p>'}</section>`).join("");}
function renderMatchPlayers(rows){$("#match-player-stats").innerHTML=rows.length?`<table><thead><tr><th>Player</th><th>Team</th><th class="numeric">Rating</th><th class="numeric">Min</th><th class="numeric">G</th><th class="numeric">A</th><th class="numeric">xG+xA</th><th class="numeric">Chances</th><th class="numeric">Touches</th><th class="numeric">Def actions</th></tr></thead><tbody>${rows.map(row=>`<tr><td><button class="player-report-link" type="button" data-player-name="${escapeHtml(row.name||"Unknown")}" data-player-club="${escapeHtml(row.team||"")}">${escapeHtml(row.name||"Unknown")}</button></td><td>${escapeHtml(row.team||"—")}</td><td class="numeric">${value(row.stats.rating,2)}</td><td class="numeric">${row.stats.minutes??"—"}</td><td class="numeric">${row.stats.goals??"—"}</td><td class="numeric">${row.stats.assists??"—"}</td><td class="numeric">${value(row.stats.xg_xa,2)}</td><td class="numeric">${row.stats.chances_created??"—"}</td><td class="numeric">${row.stats.touches??"—"}</td><td class="numeric">${row.stats.defensive_actions??"—"}</td></tr>`).join("")}</tbody></table>`:'<p class="safe-empty">Player statistics are unavailable.</p>';}
function renderMatchNetworks(networks){$("#match-networks").innerHTML=networks.map(network=>`<section><header><strong>${escapeHtml(network.team?.name||"Team")}</strong><span>Centralisation ${value(network.centralization,2)}</span></header><ol>${network.nodes.sort((a,b)=>(b.passes||0)-(a.passes||0)).map(node=>`<li><button class="player-report-link" type="button" data-player-name="${escapeHtml(node.player?.name||"Unknown")}" data-player-club="${escapeHtml(network.team?.name||"")}"><span>${escapeHtml(node.player?.name||"Unknown")}</span><small>avg ${value(node.x,1)}, ${value(node.y,1)}</small></button><b>${node.passes??"—"} passes · ${node.received??"—"} received</b></li>`).join("")}</ol></section>`).join("")||'<p class="safe-empty">Pass-network positions are unavailable.</p>';}
...
function renderPlayers(payload){
  ...
  bindPlayerReportLinks($(".player-results"));
...
async function openPlayerDossier(id,{pushHistory=true,returnView=state.productView}={}){
  state.players.scrollY=scrollY;state.players.returnView=returnView;state.players.returnKind=!$("#detail-view").hidden?"comparison":"product";setDossierTab("overview");$$(".product-view").forEach(view=>{view.hidden=true;});$("#detail-view").hidden=true;$("#player-dossier-view").hidden=false;$("#dossier-content").hidden=true;$("#dossier-loading").hidden=false;$("#dossier-back-label").textContent=state.players.returnKind==="comparison"?"Back to comparison":`Back to ${returnView}`;scrollTo({top:0});if(pushHistory)history.pushState({dossier:id},"",`#dossier=${encodeURIComponent(id)}`);
...
function closePlayerDossier(){$("#player-dossier-view").hidden=true;if(state.players.returnKind==="comparison")$("#detail-view").hidden=false;else{$$(".product-view").forEach(view=>{const name=view.dataset.productViewName||view.id.replace("-view","");view.hidden=name!==state.players.returnView;});}requestAnimationFrame(()=>scrollTo({top:state.players.scrollY,behavior:"instant"}));}

function renderPlayerDossier(player){
  $("#dossier-loading").hidden=true;$("#dossier-content").hidden=false;$("#dossier-name").textContent=player.player_name||"Unnamed player";$("#dossier-name").tabIndex=-1;$("#dossier-name").focus({preventScroll:true});$("#dossier-avatar").textContent=initials(player.player_name||"Player");$("#dossier-context").textContent=`${player.club||"Club unavailable"} · ${player.competition||"League unavailable"} · ${player.season||player.candidate_window||"Season unavailable"}`;
  $("#dossier-identity-tags").innerHTML=[player.position||"Position unavailable",player.age==null?"Age unavailable":`${value(player.age,0)} years`,player.primary_role||"Unclassified"].map(item=>`<span>${escapeHtml(item)}</span>`).join("");
  $("#dossier-scores").innerHTML=scoreTile("Current performance",value(player.current_level,1))+scoreTile("Radar interest",value(player.radar_score,1))+scoreTile("Development",signedValue(player.development,1))+scoreTile("Confidence",player.confidence||"LOW");
  const facts=[["Position",player.position||"Unavailable"],["Age",value(player.age,0)],["Minutes",player.minutes==null?"Unavailable":integer.format(player.minutes)],["Data tier",player.data_tier||"Unavailable"],["Career seasons",value(player.career_seasons,0)],["Core role coverage",player.core_role_coverage==null?"Unavailable":percentage(Number(player.core_role_coverage)*100)]];
  ...
  renderScoutReport(player);renderRoleEvidence(player);renderCurrentLevel(player);renderDevelopment(player);renderDossierEvidence(player);
}

function renderScoutReport(player){
  const components=Object.entries(player.current_level_components||{}).filter(([,item])=>item!=null).sort((a,b)=>Number(b[1])-Number(a[1]));
  const roleDimensions=(player.role_evidence?.dimensions||[]).filter(item=>item.percentile!=null).map(item=>[item.dimension,item.percentile]);
  const profile=components.length?components:roleDimensions;$("#dossier-profile-score").textContent=value(player.current_level,1);$("#dossier-profile-bars").innerHTML=profile.length?profile.slice(0,7).map(([metric,item])=>evidenceBar(metricLabel(metric),item,`${value(item,0)} percentile`)).join(""):'<p class="safe-empty">A role-relative performance profile is unavailable for this player-season.</p>';
  const name=player.player_name||"This player",role=player.primary_role||"Unclassified",season=player.season||player.candidate_window||"the selected season",sentences=[];
  sentences.push(`${name} is classified as ${role} from the available ${season} behavioural evidence, across ${player.minutes==null?"an unavailable minutes sample":integer.format(player.minutes)+" recorded minutes"}.`);
  if(components.length){const strongest=components.slice(0,3).map(([metric,item])=>`${metricLabel(metric)} (${value(item,0)}th percentile)`).join(", ");sentences.push(`The highest supported role-relative components are ${strongest}.`);}
  if(player.xg_p90!=null||player.xa_p90!=null||player.goals_p90!=null||player.assists_p90!=null){const output=[["goals",player.goals_p90],["xG",player.xg_p90],["assists",player.assists_p90],["xA",player.xa_p90]].filter(([,item])=>item!=null).map(([label,item])=>`${value(item,2)} ${label}/90`).join(", ");sentences.push(`Recorded attacking evidence: ${output}. ${player.underlying_output_label||"Underlying-versus-output context is unavailable"}.`);}
  if(player.development!=null)sentences.push(`Matched season-on-season performance movement is ${signedValue(player.development,1)} with ${player.development_confidence||"unavailable"} Development confidence; use the Development tab to inspect the evidence.`);else sentences.push("A comparable consecutive-season development reading is not available, so no trajectory is inferred.");
  $("#dossier-report-copy").innerHTML=sentences.map(sentence=>`<p>${escapeHtml(sentence)}</p>`).join("");
}
...
function renderCurrentLevel(player){
  ...
  const raw=value(player.current_level_raw,1),core=ratioPercentage(player.core_role_coverage),support=ratioPercentage(player.supporting_coverage),spatial=ratioPercentage(player.spatial_coverage),reliability=ratioPercentage(player.current_performance_reliability),league=value(player.league_current_performance,1);
  $("#dossier-current-adjustments").innerHTML=`<div><span>Raw measured evidence</span><strong>${raw}</strong></div><div><span>Core role coverage</span><strong>${core}</strong></div><div><span>Supporting / spatial</span><strong>${support} / ${spatial}</strong></div><div><span>Evidence reliability</span><strong>${reliability}</strong></div><div><span>League-relative performance</span><strong>${league}</strong></div>`;
}
...
function renderDevelopment(player){
  ...
  if(player.development==null){
    ...
    $("#dossier-development-summary").innerHTML=`<div class="development-summary"><div><span>Previous comparable</span><strong>${value(context.previous_performance_percentile,1)}</strong></div><div><span>Matched movement</span><strong class="${Number(player.development)>0?"positive":""}">${signedValue(player.development,1)}</strong></div><div><span>Matched evidence</span><strong>${ratioPercentage(context.matched_evidence_coverage)}</strong></div><div><span>Development confidence</span><strong>${escapeHtml(player.development_confidence||"UNAVAILABLE")}</strong></div></div>${changes.length?`<div class="metric-change-list">${changes.map(item=>`<span>${escapeHtml(item.metric)} <strong class="${Number(item.change)>0?"positive":""}">${signedValue(item.change,item.metric==="Minutes"?0:2)}</strong></span>`).join("")}</div>`:""}<p class="development-transition">Matched evidence only · both seasons require 900+ minutes · movement is adjusted by the weaker sample${transition.length?` · ${escapeHtml(transition.join(" · "))}`:""}</p>`;
  }
...
function renderResults(){
  ...
  $("#results-body").innerHTML=results.map(result=>`<tr tabindex="0" data-id="${escapeHtml(result.player_season_id)}"><td><span class="grade grade-${grade(result.recommendation_score).replace("+","plus").replace("-","minus")}">${grade(result.recommendation_score)}</span><small class="rec-score">${value(result.recommendation_score,0)}</small></td><td class="player-cell"><button class="player-report-link" type="button" data-dossier-id="${escapeHtml(result.player_season_id)}"><strong>${escapeHtml(result.player_name)}</strong></button><span>${escapeHtml(shortPosition(result.position))} · ${escapeHtml(result.club||"Club unavailable")} · ${escapeHtml(result.competition||"—")} · ${escapeHtml(result.candidate_window||result.season||"—")}</span><small class="why-label">${escapeHtml(matchLabels(result))}</small></td><td class="col-position">${escapeHtml(shortPosition(result.position))}</td><td class="col-club">${escapeHtml(result.club||"—")}</td><td class="col-league">${escapeHtml(result.competition||"—")}</td><td class="col-season">${escapeHtml(result.candidate_window||result.season||"—")}</td><td class="numeric">${value(result.age,1)}</td><td class="numeric col-minutes">${result.minutes==null?"—":integer.format(result.minutes)}</td><td class="numeric"><span class="profile-score">${value(result.profile_match,1)}</span></td><td class="numeric col-spatial">${value(result.spatial_match,1)}</td><td class="numeric col-xg">${value(result.xg_p90,2)}</td><td class="numeric col-xa">${value(result.xa_p90,2)}</td><td class="numeric"><span class="coverage-value">${percentage(result.comparison_coverage)}</span><small class="confidence confidence-${String(result.confidence||"low").toLowerCase()}">${escapeHtml(result.confidence||"LOW")}</small></td><td><span class="tier tier-${escapeHtml(result.data_tier)}">${escapeHtml(result.data_tier)}</span></td></tr>`).join("");
  $("#card-results").innerHTML=results.map(result=>`<article class="result-card surface" tabindex="0" data-id="${escapeHtml(result.player_season_id)}"><span class="grade">${grade(result.recommendation_score)}</span><div><button class="player-report-link" type="button" data-dossier-id="${escapeHtml(result.player_season_id)}"><strong>${escapeHtml(result.player_name)}</strong></button><p>${escapeHtml(shortPosition(result.position))} · ${escapeHtml(result.club||"Club unavailable")}</p><small>${escapeHtml(result.competition||"—")} · ${escapeHtml(result.candidate_window||result.season||"—")}</small><em>${escapeHtml(matchLabels(result))}</em></div><dl><div><dt>REC</dt><dd>${value(result.recommendation_score,1)}</dd></div><div><dt>Profile</dt><dd>${value(result.profile_match,1)}</dd></div><div><dt>Coverage</dt><dd>${value(result.comparison_coverage,0)}%</dd></div><div><dt>Tier</dt><dd>${escapeHtml(result.data_tier)}</dd></div></dl></article>`).join("");
  $$('[data-id]').filter(element=>element.closest("#results-section")).forEach(element=>{element.addEventListener("click",()=>openComparison(element.dataset.id));element.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openComparison(element.dataset.id);}});});bindPlayerReportLinks($("#results-section"));
